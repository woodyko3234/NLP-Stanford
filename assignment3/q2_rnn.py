...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        #Note that max_length might be less than the sentence length
        minimum = min(len(sentence), max_length) 
        diff = max_length - minimum
        pad_sentence = sentence[:minimum] + [zero_vector] * diff
        pad_label = labels[:minimum] + [4] * diff
        mask = [True] * minimum + [False] * diff
        ret.append((pad_sentence, pad_label, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        xs = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(xs, self.input_placeholder)
        
        embeddings = tf.reshape(embeddings, [-1, self.max_length, 
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros((input_shape, self.config.hidden_size))
        ### END YOUR CODE
