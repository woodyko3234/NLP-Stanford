...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        '''
        From the first word in the sentence, the selected data would be:
        [START, target_w, target_w+1] if window_size = 1
        '''
        padded_sentence = (
            [start] * window_size
            + sentence
        ...
        N = len(sentence)
        #loop in the sentence with the window setting
        for i, w in enumerate(padded_sentence[window_size:-window_size]):
            '''
            The range starts at window_size bcz in padded_sentence,
            the first word is padded_sentence[window_size]
            Words before it are padded items "start"
            '''
            window_label = labels[i]
            i += window_size
            window_feats = np.concatenate(padded_sentence[i - window_size: 
                                          i + window_size + 1]).tolist()
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        #Shape: (None, n_window_features, embedding_size)
        #Need to do reshape/concatenate
        embeddings = tf.reshape(embeddings, (-1, n_window_features * embedding_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        #weird thing: assignment2 asked me to use keep_prob = 1 - self.dropout_placeholder arguement
        #but it didn't work in assignment3, looked like the dropout rate is 100%
        U = tf.get_variable("U", (hidden_size, n_classes),
                            tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.Variable(tf.zeros((1, n_classes)), name = 'b2')
